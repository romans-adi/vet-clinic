...
SELECT a.name AS animal_name, v.visit_date AS visit_date, vt.name AS vet_name FROM animals AS a  JOIN  visits AS v ON a.id = v.animal_id JOIN vets AS vt ON v.vet_id=vt.id ORDER BY v.visit_date DESC LIMIT 1;
