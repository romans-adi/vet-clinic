...
SELECT
       TO_CHAR(AVG(weight_kg), 'FM999.009	') AS avg_weight
FROM
       animals;
...
SELECT
       species,
       TO_CHAR(MIN(weight_kg), 'FM999.009	') AS min_weight,
       TO_CHAR(MAX(weight_kg), 'FM999.009') AS max_weight
FROM
       animals
GROUP BY
       species;
...
SELECT
       TO_CHAR(AVG(escape_attempts), 'FM999.009') AS avg_escape_attempts
FROM
       animals
WHERE
...
SELECT
       vt.name AS vet_name,
       s.species_id AS specialization_id
FROM
       vets AS vt
       LEFT JOIN specializations AS s ON vt.id = s.vet_id
ORDER BY
       vt.name;
